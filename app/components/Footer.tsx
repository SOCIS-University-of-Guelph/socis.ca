  ...
  return (
    <footer className="w-full bg-mainblack text-mainwhite border-t border-mainwhite/10">
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8">
          <div>
            ...
            <p className="text-md mt-3 font-bold">
              Society of Computer and Information Sciences
            </p>
            <p className="text-sm opacity-60 mt-2 max-w-xs">
              The largest CS academic society at the University of Guelph!
            </p>
          </div>
              ...
              {socials.map((item) => {
                ...
                return (
                  <Link
                    ...
                    href={item.href}
                    target="_blank"
                    className={`flex items-center gap-2 transition-all rounded-md px-4 py-2 bg-white/10 justify-center md:bg-transparent md:justify-start md:px-0 md:py-1
                    ${item.hoverClass}`}
                  >
        ...
        <div className="mt-8 border-t border-white/10 pt-4 text-sm opacity-60">
          © {new Date().getFullYear()} Society Of Computer & Information Scienes
        </div>
      </div>
    </footer>
  );
